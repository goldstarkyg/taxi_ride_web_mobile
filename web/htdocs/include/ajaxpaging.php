...
///////// GET THE VALUE OF START VARIABLE////////////////
	...
	function start_ajax()
	{
		if($_REQUEST["start_ajax"])
			return	$start_ajax=$_REQUEST["start_ajax"];
		else
			return	$start_ajax=0;
	}
	
//////////////  END OF start_ajax FUNCTION///////////////////	
...
///////////// NUMERIC FUNCTION WITH RECORD DESTAIL//////////////////////////////////////
	function number_pageing($query,$record_per_page='',$pages='',$detail='',$bottom='',$simple='',$cur_page="",$query_string="",$divid="")
	{
		...
		$start_ajax=$this->start_ajax();
		
		$order=$_GET['order'];
		$this->query.=" limit $start_ajax,".$this->record_per_page;  
		$result=$this->runquery($this->query);
		$total= mysql_affected_rows();
		
		$total_pages=ceil($totalrows/$this->record_per_page);
		$current_page=($start_ajax+$this->record_per_page)/$this->record_per_page;
		...
		if($start_ajax>0)
		{ 
			//$this->tbl.="<a class='paging' href='".$cur_page."?start_ajax=".($start_ajax-$this->record_per_page).$query_string."' class='boldbluelink' onMouseOver=\"smsg('Previous Page');return document.prs_return\" onMouseOut=\"nosmsg('Done');return document.prs_return\">&lt;&lt;  Previous</a>&nbsp;&nbsp;"; 
			
			$this->tbl.="
			<li class='num_off' onclick=call_ajax_view('".$cur_page."','start_ajax=".($start_ajax-$this->record_per_page).$query_string."','".$divid."')>PREV</li>";
		...
		if($detail!="N" and $simple !="N")
			$this->tbl.="Result ".($start_ajax+1)." - ".($start_ajax+$total)." of ".$totalrows." Records<BR>";
		if($simple!='N')
		{
			...
			for($i=$start_loop;$i<$end_loop;$i++) 
			{
				...
				{ 
					//$this->tbl.="<a class='paging' href='".$cur_page."?start_ajax=".($i-1)*$this->record_per_page.$query_string."' class='boldbluelink' onMouseOver=\"smsg('View Page Number $i');return document.prs_return\" onMouseOut=\"nosmsg('Done');return document.prs_return\">".$i."</a>&nbsp;&nbsp;"; 
					
					$this->tbl.="<li class='num_off' onclick=call_ajax_view('".$cur_page."','start_ajax=".($i-1)*$this->record_per_page.$query_string."','".$divid."')>".$i."</li>";
		...
		if($start_ajax+$this->record_per_page<$totalrows) 
		{ 
			//$this->tbl.="<a class='paging' href='".$cur_page."?start_ajax=".($start_ajax+$this->record_per_page).$query_string."' class='boldbluelink' onMouseOver=\"smsg('Next Page');return document.prs_return\" onMouseOut=\"nosmsg('Done');return document.prs_return\">Next  &gt;&gt;</a>"; 
			
			$this->tbl.="<li class='num_off' onclick=call_ajax_view('".$cur_page."','start_ajax=".($start_ajax+$this->record_per_page).$query_string."','".$divid."')>NEXT</li>";
