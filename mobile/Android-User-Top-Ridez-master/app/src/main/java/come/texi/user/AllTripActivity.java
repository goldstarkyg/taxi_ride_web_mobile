...
import come.texi.user.adapter.AllTripAdapter;
                            ...
                            if (resObj.getString("status").equals("success")) {
                                ...
                                for (int t = 0; t < tripArray.length(); t++) {
                                    ...
                                    if(trpObj.has("payment_status"))
                                        ...
                                        allTripFeed.setPaymentStatus(""); //Change by U
                                    }
...
public void FilterAllTrips(final int offset, final String filter){
    ...
    Log.e("loadTripsUrl", "loadTripsUrl FilterString= " +Url.loadTripsFiltersUrl+"=="+FilterString+"=="+userPref.getString("id", "")+"=="+String.valueOf(offset));
...
public void clickDetailTrip(int position) {
    ...
    if(allTripArray.size() > 0) {
        ...
        if(Common.allTripFeeds.getStatus().equals("3")||Common.allTripFeeds.getStatus().equals("7") || Common.allTripFeeds.getStatus().equals("8")) {
            //TrackTheTruck
            Intent di = new Intent(AllTripActivity.this,TrackTruckActivity.class);
            di.putExtra("which_screen",1);
...
//                        .setMultipartParameter("uid", userPref.getString("id", ""))
    ...
    public void onResume() {
        ...
        IntentFilter filter = new IntentFilter("come.texi.user.AllTripActivity");

        Common.receiver = new BroadcastReceiver() {
            @Override
            public void onReceive(Context context, Intent intent) {

                System.out.println("Payment Procedure >>>Broadcast");
            ...
            registerReceiver(Common.receiver, filter);
        }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if(requestCode == 1 || requestCode == 2){
            if(data != null){
                ...
                AllTripFeed allTripFeed = allTripArray.get(position);
                allTripFeed.setPaymentType(PaymentType);
                allTripFeed.setPaymentStatus(PaymentStatus);
                allTripFeed.setIsUploadReview(reviews);
