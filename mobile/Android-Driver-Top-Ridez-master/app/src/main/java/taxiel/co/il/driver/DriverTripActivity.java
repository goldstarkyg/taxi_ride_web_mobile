...
import taxiel.co.il.driver.Adapter.DriverAllTripAdapter;
...
public class DriverTripActivity extends AppCompatActivity implements DriverAllTripAdapter.OnAllTripClickListener  {
    ...
    double latitude;
    double longitude;
//    private static final String SERVER_IP = "http://139.59.154.174:4040";
//    private Socket mSocket;
   ...
   boolean isRefreshData=false;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        isRefreshData=true;
        SocketSingleObject.get(this).getSocket().connect();
                    ...
                    if(gps.canGetLocation()) {
                        try {
                            if(Common.socket!=null && Common.socket.connected()){
                                    System.out.println("Ping socket already connected");
                            }else{
                                Common.socket=null;
                                SocketSingleObject.instance=null;
                                Common.socket=SocketSingleObject.get(DriverTripActivity.this).getSocket();
                                Common.socket.connect();
                                System.out.println("Ping socket again connected");
                            }
                        ...
                        Common.SocketFunction(DriverTripActivity.this,Common.socket,driver_status,latitude,longitude,common,userPref);
                    }else{
...
//                        Common.socket.disconnect();
                        SocketSingleObject.get(DriverTripActivity.this).getSocket().disconnect();
                        Common.socket=null;
                        SocketSingleObject.instance=null;
        ...
        Log.e("OkaySwitzerland","id_device_token = "+userPref.getString("id_device_token",""));

        if(userPref.getString("id_device_token","").equals("") || userPref.getString("id_device_token","").equals("1")) {
            if(Utility.isNetworkAvailable(DriverTripActivity.this)) {
                if(Common.device_token != null && !Common.device_token.equals("")){
                    ...
                    new CallUnsubscribe(DriverTripActivity.this, Common.device_refresh_token).execute();
                }
    ...
    public void AcceptCabBookin(final int position) {
        if(DriverAllTripArray.size() > 0) {
            ...
            Common.UpdateSocketDriverStatus(Common.socket,userPref,"2");
            ...
            Ion.with(DriverTripActivity.this)
                    ...
                    .setCallback(new FutureCallback<JsonObject>() {
                        ...
                        public void onCompleted(Exception error, JsonObject result) {
                            ...
                            Log.e("Okay Swiss", "Accept booking = " + result + "==" + error);
                            loader.cancel();
                            if (error == null) {
                                try {
                                    JSONObject resObj = new JSONObject(result.toString());

                                    if(resObj.has("code") && resObj.getString("code").equals("700")){
                                    ...
                                    if(resObj.getString("status").equals("success")) {
                                        ...
                                        DriverStatus.putString("driver_status","busy");
                                        DriverStatus.commit();
                                ...
                                if (error == null) {
                                    ...
                                    try {
                                        ...
                                        if(resObj.getString("status").equals("success")) {
                                            ...
                                            driver_status.setChecked(true);
                                            driver_status.setClickable(false); // By Uttam when rejected.
                                            switch_driver_status.setText(getResources().getString(R.string.off_duty));
                            ...
                            try {
                                ...
                                if (resObj.getString("status").equals("success")) {
                                    ...
                                    for (int t = 0; t < tripArray.length(); t++) {
                                        ...
                                        allTripFeed.setPaymentType(trpObj.getString("payment_type"));
                                        allTripFeed.setIsUploadReview(trpObj.getString("isflag"));

                                        if(trpObj.has("payment_status"))
                                            allTripFeed.setPaymentStatus(trpObj.getString("payment_status"));
                                        else{
                                            allTripFeed.setPaymentStatus("");
                                        }
                                        DriverAllTripArray.add(allTripFeed);

                                        Log.e("PaymentStatus","PaymentStatus with filter >>"+trpObj.getString("payment_status"));
                                    }
            ...
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    if (Common.socket != null)
                        if (Common.socket.connected() == false) {
                            Log.d("connected", "connected error= " + Common.socket.connected());
                            //socketConnection();
                        }else
                        {
                            Log.d("connected", "connected three= " + Common.socket.connected());
                        }
                }
            });
        }
                            ...
                            try {
                                ...
                                if (resObj.getString("status").equals("success")) {
                                    ...
                                    for (int t = 0; t < tripArray.length(); t++) {
                                        ...
                                        allTripFeed.setIsUploadReview(trpObj.getString("isflag"));
                                        if(trpObj.has("payment_status"))
                                            allTripFeed.setPaymentStatus(trpObj.getString("payment_status")); //Change by U
                                        else{
                                            allTripFeed.setPaymentStatus("");
                                        }

                                        Log.e("PaymentStatus","PaymentStatus without filter >>"+trpObj.getString("payment_status"));
    ...
    public class CallUnsubscribe extends AsyncTask<String, Void, String > {
        ...
        protected String doInBackground(String... args) {
            ...
            try {
                ...
                passObj.put("token",Common.device_token);
        ...
        protected void onPostExecute(String result) {
            ...
            if(result.contains("HTTP/1.1 200 OK")){
                if(Common.device_token != null && !Common.device_token.equals("")){
                    ...
                    new CallSubscribe(activity,Common.device_refresh_token).execute();
                }
...
//                httppost.setEntity(se);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if(requestCode == DETAIL_REQUEST){
            if(data != null && DriverAllTripArray != null) {
                ...
                driverAllTripFeed.setPaymentType(data.getStringExtra("payment_type"));
                driverAllTripFeed.setPaymentStatus(data.getStringExtra("payment_status")); //Change by U
                Log.e("OkaySwiss","onActivityResult"+data.getStringExtra("reviews"));
                if(data.getStringExtra("reviews")!=null && !data.getStringExtra("reviews").equals("") && data.getStringExtra("reviews").equals("1")){
                    driverAllTripFeed.setIsUploadReview(data.getStringExtra("reviews"));
                }else if(data.getStringExtra("reviews")!=null && !data.getStringExtra("reviews").equals("") && data.getStringExtra("reviews").equals("0")){
                    driverAllTripFeed.setIsUploadReview(data.getStringExtra("reviews"));
                }
                DrvAllTripAdapter.notifyItemChanged(position);
            }
    ...
    protected void onResume() {
        ...
        if(userPref.getString("driver_status","").equals("busy")){
            ...
            System.out.println("Driver Status >2>>busy"+isRefreshData);
            driver_status.setChecked(true);
            driver_status.setClickable(false); //Change By Uttam -false
          switch_driver_status.setText(getResources().getString(R.string.on_duty));
            ...
            if(gps.canGetLocation()) {
                try {
                        ...
                        System.out.println("Driver Status >>>create new instance socket"+Common.socket);
            ...
            new AlertDialog.Builder(this)
                    ...
                    .setPositiveButton(android.R.string.yes, new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface arg0, int arg1) {
                            ...
                            Common.counter.cancel();
                            DriverTripActivity.super.onBackPressed();
                        }
                    }).create().show();
        }
    }
...
//                Toast.makeText(getApplicationContext(), "Connected", Toast.LENGTH_LONG).show();
                ...
                OkswissApplication.activityResumed(DriverTripActivity.this);
            }else{
